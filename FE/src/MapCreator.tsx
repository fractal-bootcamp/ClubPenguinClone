...
const calculateNewEntityMap = (
  collisionMap: boolean[],
  prevEntityMap: EntityMap,
  entity: Entity
) => {
  const updatedMap = collisionMap.map((collision, index) => {
    return {
      ...
      entities: collision
        ? [...prevEntityMap[index].entities, entity]
        : prevEntityMap[index].entities,
    };
  });
  return updatedMap;
};

const initialEntityMap: EntityMap = Array.from(
  { length: cellsX * cellsY },
  (_, index) => {
    return {
      x: index % cellsX,
      y: Math.floor(index / cellsX),
...
const MapCreator = () => {
  ...
  const [collisionMap, setCollisionMap] = useState<boolean[]>(
    new Array(cellsX * cellsY).fill(false)
  );

  const [collisionMode, setCollisionMode] = useState<"add" | "remove" | null>(
    null
  );
